dat1 <- read.csv("data.csv", stringsAsFactors = FALSE) %>%
...
# Function 1 take formatted data, does poissn regression and then returns parameteres in as list for each team
...
# another team goal---> this assumption might not necessarily be true in real game
...
game_result <- function(H,A){
    ...
    plot_pie <<- plot_ly(game_wheel,values = ~Val,labels = ~teams,
                 type= 'pie',
                 marker= list(colors=c("red", "yellow", "green"),
    ...
    final_graph1 <<- ggplot()+geom_bar(aes(x=h_goal_sim),stat="count",
                                    width = 1,color="white",fill=scales::alpha('violet',.5))+
